...
export async function getAIRecommendationAction(
  ...
  try {
    ...
    // TODO - test "just want to change colour of my car - paint it into anime design"
    ...
    1. reply with hmtl but without \`\`\`html
...
<span class="block tablet:inline tablet:ml-1 tablet:before:content-['-']">But maybe you can share an image of your car [inside or outside (in this case right is "outside" because user specified inside)] so I understand it better?</span>
</div>
      \`\`\` 
    4. Once user specified he's needs (inside or outside or both) then you reply based on user needs
      \`\`\`
